...
groupFigures <- function(figure1, figure2, figure3, reduce.x = TRUE){
    ...
    figure1 <- figure1 + ggtitle("Study A: mixed model") + xlab("") + theme(text = element_text(size = 10), legend.position="none")
    figure2 <- figure2 + ggtitle("Study B: single factor model") + xlab("") + theme(text = element_text(size = 10), legend.position="none")
    if(reduce.x == TRUE){
        figure2 <- figure2 + theme(axis.text.x=element_text(color=x.factor))
    }
    figure3 <- figure3 + ggtitle("Study C: two latent variables model") + theme(text = element_text(size = 10))
